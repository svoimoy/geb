...
func make_type(dsl_ctx interface{}, dsl_map map[string]*dsl.Dsl, design_data map[string]interface{}) ([]Plan, error) {
	...
	logger.Debug("Making Type plan", "dsl_map", dsl_map, "ctx_dsl", ctx_dsl, "ctx_path", ctx_path)
	...
	for d_key, D := range dsl_map {
		...
		for g_key, G := range D.Generators {
			...
			for _, R := range repeats {
				...
				logger.Info("Doing Repeated Field: '" + R.Name + "'")
				...
				for _, t_pair := range R.Templates {
					...
					for idx, val := range c_slice {
						...
						if err != nil {
							return nil, errors.Wrap(err, "in make_types\n")
						}
