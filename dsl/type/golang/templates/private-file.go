...
In template: type/golang/template/private-file.go
also need to case receiver type name based on private or not. need to look up type though...
*/
func ({{lower (substr RC.parent 0 1)}} *{{camelT RC.parent}}) {{camel name}}({{#if args}}{{> pkg/golang/func-args.go args ~}}{{/if}} ) {{#if return}}{{> pkg/golang/func-return.go return ~}}{{/if ~}} {
